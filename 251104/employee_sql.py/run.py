from employee_sql import Employee
import repo

def menu():
    option_str = choice = int(input(option_str))
    ...
    if choice == 1:
        ...
        if old_employee:
            ...
            repo.add_employee(
                Employee(id=id, name=name, job_title=job_title, salary=salary))
        ...
        if not old_employee:
            ...
            print(f'Employee {old_employee.name} salary updated successfully')
    elif choice == 4:
        id = input('ID:')
        old_employee = repo.search_employee(id)
        if not old_employee:
